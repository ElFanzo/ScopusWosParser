...
colors = ['purple', 'gold', 'deepskyblue', 'darkorange', 'red', 'lightskyblue',
          'greenyellow', 'brown', 'black']
...
def hist_h(labels, values):
    ...
    for rect in rects:
        h = rect.get_height()
        w = rect.get_width()
        ax.text(w + 5, rect.get_y() + h/2.5, '{}'.format(int(w)),
                ha='center', va='center')
...
def hist_v(labels, values, group=False):
    ...
    for rect, j in zip(rects, val):
        ...
        ax.text(rect.get_x() + w/2, h + 2, text,
                ha='center', va='center')
...
def diag_circle(x, y):
    global colors
    porcent = [100.*i/sum(y) for i in y]
...
if __name__ == '__main__':
    ...
    query1 = 'select institute, count(*) as num from teachers '\
             'group by institute order by num desc'
    query2 = 'select institute, count(*) as num from teachers '\
             'where h_index <> "None" group by institute '\
             'order by num desc'
    query3 = 'select a.institute, b.num * 100.0 / count(*) as perc '\
    ...
    query4 = 'select unit, count(*) as num from teachers where institute = "%s" '\
             'group by unit order by num desc' % inst
    query5 = 'select unit, count(*) as num from teachers where institute = "%s" '\
             'and h_index <> "None" group by unit order by num desc' % inst
    query6 = 'select a.unit, b.num * 100.0 / count(*) as perc from teachers a, (%s) b '\
             'where a.unit = b.unit and institute = "%s" group by a.unit '\
             'order by perc desc' % (query5, inst)

    query7 = 'select institute, round(max(h_index), 1) as max_h from teachers '\
             'where h_index <> "None" group by institute order by max_h desc'
    query8 = 'select h_index, count(*) as num from teachers where h_index <> "None" '\
