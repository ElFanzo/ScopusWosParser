...
def get_teachers(page):
    g = Grab()
    url = 'https://kpfu.ru/main_page?p_sub=7860&p_order=1&p_id=0&p_period=on&p_rec_count=435&p_page=%d' % page
    g.go(url, timeout=90)

    teachers = g.doc.select('//*[@id="ss_content"]/div[2]/div/div[2]/table[2]/tr/td/span/a')

    result = ''
    for man in teachers:
        g.go(man.select('@href').text())
        try:
            info = g.doc.select('//*[@id="ss_content"]/div/div/div[2]/div[2]/div[1]/div/table').text()
        except DataNotFound:
            pass
        else:
            try:
