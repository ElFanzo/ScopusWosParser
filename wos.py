...
def get_h_wos(id):
    ...
    try:
        href = g.doc.select('//*[@id="view_citation_report_image"]/div/div/a/@href').text()
    except DataNotFound:
        return 'None'
...
def scrape_wos(file=None):
    if not file:
        ctx = DataCtx()
        teachers_ids = ctx.select('select researcher_id from teachers where researcher_id <> "None"')
    else:
        with open(file) as f:
            rows = f.read()
    ...
    if not file:
        ...
        ctx.execute_many('update teachers set h_index = ? where researcher_id = ?', params)
    else:
